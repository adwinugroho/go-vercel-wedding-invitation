...
type (
	RSVPInterface interface {
		List(offset int, limit int, isAttending string) ([]model.Reservation, error)
...
func (s *rsvpImp) List(offset, limit int, isAttending string) ([]model.Reservation, error) {
	// ctx := context.Background()
	data, err := s.repo.ListWithSupabaseClient(offset, limit, isAttending)
	if err != nil {
