...
type (
	RSVPInterface interface {
		...
		List(ctx context.Context, offset, limit int) ([]model.Reservation, error)
...
func (c *rsvpImp) Insert(ctx context.Context, data model.Reservation) (string, error) {
	...
	_, err := c.DB.Exec(ctx, query, data.Name, data.IsAttending, data.GuestCount, data.CreatedAt)
	if err != nil {
...
func (c *rsvpImp) List(ctx context.Context, offset, limit int) ([]model.Reservation, error) {
	var results []model.Reservation
	query := `SELECT id, name, is_attending, guest_count FROM tb_rsvp WHERE is_published = true LIMIT $1 OFFSET $2`
	...
	for rows.Next() {
		...
		err = rows.Scan(&result.ID, &result.Name, &result.IsAttending, &result.GuestCount)
		if err != nil {
